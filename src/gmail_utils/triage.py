...
from gmail_utils.schemas import TriageState, Router
from gmail_utils.utils import parse_gmail, format_gmail_markdown, user_profile
from gmail_utils.memory import get_memory, update_memory
from gmail_utils.prompts import triage_system_prompt, triage_user_prompt, default_triage_instructions


LLM_ROUTER = init_chat_model("openai:gpt-4.1-mini", temperature=0.0).with_structured_output(Router)
...
def triage_router(state: TriageState, store: BaseStore) -> Command[Literal["triage_interrupt_handler", "response_agent", "__end__"]]:
    ...
    email = format_gmail_markdown(subject, author, to, email_thread, email_id)

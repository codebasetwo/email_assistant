...
from gmail_utils.schemas import State
from gmail_utils.memory import get_memory, update_memory
from gmail_utils.prompts import (default_cal_preferences, default_response_preferences, 
                                     agent_system_prompt_hitl_memory, GMAIL_TOOLS_PROMPT,
                                     MEMORY_UPDATE_INSTRUCTIONS_REINFORCEMENT,
                                        )
from gmail_utils.utils import user_profile, format_gmail_markdown, format_for_display, parse_gmail
from gmail_utils.gmail_tools import  mark_as_read, send_email_tool, schedule_meeting_tool, check_calendar_tool, Question, Done, fetch_emails_tool
    ...
    return {
        "messages": [
           LLM_WITH_TOOLS.invoke(
                [
                    {"role": "system", "content": agent_system_prompt_hitl_memory.format(tools_prompt=GMAIL_TOOLS_PROMPT,
...
def should_continue(state: State, store: BaseStore) -> Literal["interrupt_handler", "mark_as_read_node"]:
    ...
    if last_message.tool_calls:
        for tool_call in last_message.tool_calls: 
            ...
            #    result.append({"role": "tool", "content": "the email has been sent successfully", "tool_call_id": tool_call["id"]})
            #     update_memory(store, ("email_assistant", "conditional_preferences"), [{
            #         "role": "user",
            #         "content": f"User marked the message as sent successfully. Here is the email generated by the assistant: {tool_call["args"]}. Follow all instructions above, and remember: {MEMORY_UPDATE_INSTRUCTIONS_REINFORCEMENT}."
            #     }]) 
...
def mark_as_read_node(state: State):
    ...
    *_, email_id = parse_gmail(email_input)
